...
import {
  collectionRuneFilters,
  collectionSortModes,
  getCollectionSortLabel,
} from "../utils/collectionControls";
  ...
  return (
    <section className="space-y-3">
      <div className="space-y-1">
        <p className="font-meta text-[0.65rem] uppercase tracking-[0.22em] text-[var(--collection-ink-soft)]">
          {title}
        </p>
        {description ? (
          <p className="text-[0.74rem] leading-5 text-[var(--collection-ink-muted)]">
            {description}
          </p>
        ) : null}
      </div>
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        !active &&
          "border-white/10 text-[#d0c5af] shadow-none hover:border-primary/40 hover:text-primary",
      )}
      aria-pressed={active}
    >
      ...
      <Badge
        ...
        className="h-5 min-w-7 justify-center px-2 text-[0.58rem] tracking-[0.12em]"
      >
        {count}
      </Badge>
    </Button>
    ...
    <aside className="lg:sticky lg:top-6">
      <div className="space-y-5 rounded-[0.25rem] border border-white/8 bg-[linear-gradient(180deg,rgba(31,34,39,0.92),rgba(12,14,17,0.98))] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm md:p-6">
        <div className="space-y-2">
          <p className="font-meta text-[0.65rem] uppercase tracking-[0.28em] text-[var(--collection-ink-soft)]">
            Collection Filters
          </p>
          <h2 className="text-xl tracking-[0.04em] text-[var(--collection-primary)]">
            Rune Icon Filters
          </h2>
          <p className="max-w-xs text-sm leading-6 text-[var(--collection-ink-muted)]">
            Narrow the archive, cycle the ledger sort, and keep the found pictos
            in view.
          </p>
        </div>
        ...
        <SidebarSection
          title="Search"
          description="Match names, effects, or locations in the archive."
        >
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="rounded-lg border border-white/8 bg-black/20 p-3">
            ...
            <p className="mt-1 text-2xl text-white">{foundCount}</p>
          </div>
        </div>
        ...
        <SidebarSection
          ...
          description={
            activeRuneCount > 0
              ? `${activeRuneCount} rune group${activeRuneCount === 1 ? "" : "s"} active.`
              : "Select one or more rune families."
          }
        >
          <div className="space-y-2">
        ...
        <SidebarSection
          title="Sort"
          description={`Current order: ${getCollectionSortLabel(sortMode)}`}
        >
        ...
        <SidebarSection
          title="Display"
          description="Switch between a detailed record and a compact ledger."
        >
