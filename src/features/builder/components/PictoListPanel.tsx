...
export function PictoListPanel({
  ...
  const suggestionMap = new Map(
    suggestions.map((suggestion) => [suggestion.lumina.id, suggestion]),
  );
  ...
  const formatStats = (picto: Picto) => {
    ...
    if (picto.stats.critRate > 0)
      chunks.push(`+${picto.stats.critRate} Crit Rate`);
  ...
  const filterMatchesPattern = (
    ...
    patterns.some((pattern) => {
      ...
      return patternMatchesFilter(
        pattern.name,
        filter as "AP" | "Status" | "Buff" | "Negative",
      );
    });
  ...
  const filteredSuggestions =
    activeFilters.length === 0
      ...
      : suggestions.filter((s) =>
          activeFilters.every((f) => filterMatchesPattern(f, s.patterns)),
        );
  ...
  const filteredLuminas = luminas.filter((lumina) => {
    ...
    return activeFilters.every((filter) =>
      filterMatchesPattern(filter, suggestion.patterns),
    );
  });
  const filteredPictos = pictos
    ? pictos
        .filter((picto) => picto.name.toLowerCase().includes(normalizedSearch))
        .sort((a, b) => {
  ...
  return (
    <section className="max-h-none overflow-y-visible rounded-xl border border-[#e5e7eb] bg-white p-4 min-[1181px]:max-h-[70vh] min-[1181px]:overflow-y-auto">
      <input
        type="search"
        value={searchQuery}
      ...
      {mode === "pictos" ? (
        <>
          <h3 style={{ marginTop: 0, color: "#111827" }}>
            All Pictos ({filteredPictos.length})
          </h3>
          {filteredPictos.map((picto) => {
            ...
            return (
              ...
              >
                ...
                >
                  <strong style={{ color: "#1f2937" }}>Stats:</strong>{" "}
                  {formatStats(picto)}
                </p>
                <p
                  style={{
                ...
                >
                  <strong style={{ color: "#1f2937" }}>Lumina Effect:</strong>{" "}
                  {getLuminaEffectText(linkedLumina)}
                </p>
                <div
                  style={{
                    display: "flex",
        ...
        <>
          <h3 style={{ marginTop: 0, color: "#111827" }}>
            All Luminas ({filteredLuminas.length})
          </h3>
          {filteredLuminas.map((lumina) => {
            const variants = pictosGroupedByLumina[lumina.id] ?? [];
            const equippedPicto = selectedPictos.find(
              (p) => p.luminaId === lumina.id,
            );
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: "0.75rem",
                  borderRadius: "10px",
                  background: isProvidedByPicto
                    ? "#e9f9ef"
                    : isAddedLumina
                      ? "#eff6ff"
                      : "#f8fafc",
                  border: isProvidedByPicto
                    ? "2px solid #16a34a"
                    : isAddedLumina
                      ? "2px solid #2563eb"
                      : "1px solid #d1d5db",
              ...
              >
                ...
                >
                  <strong style={{ color: "#1f2937" }}>Effect:</strong>{" "}
                  {effectText}
                </p>
                <p
                  style={{
                ...
                >
                  <strong style={{ color: "#1f2937" }}>Lumina Cost:</strong>{" "}
                  {luminaCost}
                </p>
                <div
                  style={{
                ...
                >
                  <span style={{ color: "#6b7280", fontSize: "0.8rem" }}>
                    {isProvidedByPicto
                      ? "Included in graph for free"
                      : isAddedLumina
                        ? "Counts toward lumina cost"
                    ...
                    style={{
                      ...
                      color: isProvidedByPicto
                        ...
                        : isAddedLumina
                          ...
                          : "#1f2937",
                      fontWeight:
                        isProvidedByPicto || isAddedLumina ? 700 : 500,
      ...
      {suggestions.length > 0 && (
        <div style={{ marginTop: "1.4rem" }}>
          <h3 style={{ color: "#111827", marginBottom: "0.5rem" }}>
            Suggested Luminas (Synergy)
          </h3>

          {filteredSuggestions.length === 0 && (
            <p style={{ margin: 0, fontSize: "0.84rem", color: "#6b7280" }}>
              No suggestions match the active filters.
            </p>
          )}
